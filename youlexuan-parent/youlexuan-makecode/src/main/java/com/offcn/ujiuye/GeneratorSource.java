...
public class GeneratorSource {
	...
	public static void makeService(String modelName,String tableName, String tableComment, String primarykey) {
		...
		try {
			String src = IOUtils
					.toString(new FileInputStream(loader.getResource("template/[Table2]Service.java").toURI().getPath()), "utf-8");

//			String src = IOUtils.toString(new FileInputStream(".\\src\\main\\resources\\template\\[Table2]Service.java"), "utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File(
					"youlexuan-makecode/src/main/java/" + path_1 + "/" + path_2 + "/" +modelName+ "/service/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
			// 创建源码
	...
	public static void makeServiceImpl(String modelName,String tableName, String tableComment, List<String> columns, String primarykey) {
		...
		for (String str : columns) {
			try {
				String src = IOUtils.toString(new FileInputStream(loader.getResource("template/条件查询.String.txt").toURI().getPath()),
						"utf-8");
...
//						"utf-8");
		...
		try {
			String src = IOUtils.toString(
					new FileInputStream(loader.getResource("template/[Table2]ServiceImpl.java").toURI().getPath()), "utf-8");
...
//					new FileInputStream(".\\src\\main\\resources\\template\\[Table2]ServiceImpl.java"), "utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File("youlexuan-makecode/src/main/java/" + path_1 + "/" + path_2 + "/" +modelName
					+ "/service/impl/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
	 ...
	 * @param tableComment
	 */
	public static void makeController(String modelName,String tableName, String tableComment, String primarykey) {
		String srcJava = "";
		// 替换字符串中的占位符
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("Table2", StringReplace.removeUp(tableName));
		...
		try {
			String src = IOUtils.toString(
					new FileInputStream(loader.getResource("template/[Table2]Controller.java").toURI().getPath()), "utf-8");
...
//					new FileInputStream(".\\src\\main\\resources\\template\\[Table2]Controller.java"), "utf-8");
	...
	public static void makeJsController(String tableName, String tableComment) {
		...
		try {
			String src = IOUtils
					.toString(new FileInputStream(loader.getResource("template/[table2]Controller.js").toURI().getPath()), "utf-8");
...
//					.toString(new FileInputStream(".\\src\\main\\resources\\template\\[table2]Controller.js"), "utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File("youlexuan-makecode/src/main/webapp/js/controller/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
			// 创建源码
	...
	public static void makeJsService(String tableName, String tableComment) {
		...
		// 读取Service模板文件
		try {
			String src = IOUtils.toString(new FileInputStream(loader.getResource("template/[table2]Service.js").toURI().getPath()),
					"utf-8");
...
//					"utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File("youlexuan-makecode/src/main/webapp/js/service/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
			// 创建源码
	 ...
	 * @param columns
	 */
	public static void makeHtml(String tableName, String tableComment, List<String> columns) {
		String srcJava = "";
		// 替换字符串中的占位符
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("Table2", StringReplace.removeUpFromTwo(tableName));
		...
		for (String str : columns) {
			try {
				...
				String titlesrc = IOUtils.toString(new FileInputStream(loader.getResource("template/页面表格标题.txt").toURI().getPath()),
						"utf-8");
				String tablesrc = IOUtils.toString(new FileInputStream(loader.getResource("template/页面表格内容.txt").toURI().getPath()),
						"utf-8");
				String formsrc = IOUtils
						.toString(new FileInputStream(loader.getResource("template/编辑表单.nokey.txt").toURI().getPath()), "utf-8");
...
//						.toString(new FileInputStream(".\\src\\main\\resources\\template\\编辑表单.nokey.txt"), "utf-8");
		...
		try {
			...
			String src = IOUtils.toString(new FileInputStream(loader.getResource("template/[table2].html").toURI().getPath()),
					"utf-8");
...
//					"utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File("youlexuan-makecode/src/main/webapp/admin/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
			// 创建源码
